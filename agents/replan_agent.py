...
DO NOT ask for more of the same data type."""
                print(f"⚠️ {self.name}: Duplicate result detected - warning LLM.")
...
- Maximum 3 total execution steps (including past and planned future) allowed.
...
Respond with JSON only."""
        ...
        try:
            ...
            if isinstance(output.action, Response):
                ...
                if total_steps > 3:
                    print(f"⚠️ {self.name}: Too many total steps planned ({total_steps} > 3). Forcing synthesis instead.")
                    return {"ready_for_synthesis": True}
            ...
            print(f"❌ {self.name}: Replanning failed: {e}. Forcing synthesis to avoid infinite loop.")
            # Fallback to synthesis if replanning fails to prevent infinite loops
            return {"ready_for_synthesis": True}